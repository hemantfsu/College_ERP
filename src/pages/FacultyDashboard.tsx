          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    if (Object.keys(attendanceRecords).length < selectedClassForAttendance.students) {
                      if (!confirm(`You have marked ${Object.keys(attendanceRecords).length}/${selectedClassForAttendance.students} students. Save anyway?`)) {
                        return;
                      }
                    }
                    const presentCount = Object.values(attendanceRecords).filter(v => v === true).length;
                    const absentCount = Object.values(attendanceRecords).filter(v => v === false).length;
                    
                    addNotification({
                      id: `notif-${Date.now()}`,
                      type: 'success',
                      message: `Attendance saved for ${selectedClassForAttendance.name}`,
                      timestamp: new Date().toISOString(),
                      read: false
                    });
                    
                    alert(`✅ Attendance Saved Successfully!\n\nClass: ${selectedClassForAttendance.name}\nPresent: ${presentCount}\nAbsent: ${absentCount}\nTotal: ${selectedClassForAttendance.students}\n\nAttendance percentage: ${((presentCount / selectedClassForAttendance.students) * 100).toFixed(1)}%`);
